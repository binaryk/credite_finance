...
class BanciGrid extends \Credite\GridsRecord
{

    public function __construct($id)
    {
        parent::__construct($id);
        $this->view           = 'banci.index';
        $this->icon           = 'admin/img/icons/dt/settings.png';
        $this->caption        = 'Bănci';
        $this->toolbar        = 'banci.toolbar';
        $this->name           = 'dt';
        ...
        $this->row_source     = 'grid_banci_source';
        $this->rows_source_sql 				= 'SELECT
                                                *
                                                FROM banci
                                                :where: :order:';
        $this->count_filtered_records_sql 	= 'SELECT COUNT(*) as cnt FROM banci :where:';
        $this->count_total_records_sql     	= 'SELECT COUNT(*) AS cnt FROM banci';
        $this->columns        = [
            '1' => [
                'id'        => '#',
                'orderable' => 'no',
    ...
    {
        return self::$instance = new BanciGrid('grid_banci');
    }
